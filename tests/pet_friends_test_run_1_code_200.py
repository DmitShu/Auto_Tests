...
def test_add_new_pet_with_valid_data(get_api_key, name=add_name, animal_type=add_animal_type,
...
def test_delete_pet_valid_user(get_api_key):
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(get_api_key, tmp_name, tmp_animal_type, tmp_age, tmp_pet_photo)
        _, my_pets = pf.get_list_of_pets(get_api_key, "my_pets")
...
def test_add_new_pet_simple_with_valid_data(get_api_key, name=add_smpl_name, animal_type=add_smpl_animal_type,
                                     age=add_smpl_age):
    """Проверяем что можно добавить питомца без фото с корректными данными из settings.py"""

    # Добавляем питомца
    status, result = pf.add_new_pet_simple(get_api_key, name, animal_type, age)
...
def test_update_pet_info_with_valid_data(get_api_key, name=upd_name, animal_type=upd_animal_type, age=upd_age):
...
def test_add_pet_photo_with_valid_data(get_api_key, pet_photo2=add_pet_photo2, pet_photo3=add_pet_photo3):
    ...
    if len(my_pets['pets']) > 0:
        ...
        status1, result = pf.add_pet_photo(get_api_key, id_orig, pet_photo2)
        photo_orig = result['pet_photo']
        status2, result = pf.add_pet_photo(get_api_key, id_orig, pet_photo3)
